...
// Match by model id prefix, FIRST MATCH WINS — keep specific prefixes above
// generic ones (claude-opus-4-8 must precede claude-opus-4, which carries the
// old 4.0/4.1 $75 tier). Updated 2026-07-12 from the claude-api skill's model
// table (cached 2026-06-24); re-check https://www.anthropic.com/pricing on new
// releases. Sonnet 5 sticker is $15 (intro $10/MTok through 2026-08-31).
const MODEL_OUTPUT_PRICE_PER_M = [
  ...
  ["claude-opus-4", 75.0], // legacy Opus 4.0/4.1 tier
...
function parseSession(filePath) {
  ...
  try {
    ...
    return { outputTokens: 0, cacheReadTokens: 0, turns: 0, model: null };
  }
...
function formatHistory({
  ...
  const usdLine =
    estSavedUsd > 0
      ? `Est. saved (USD):      ~${formatUsd(estSavedUsd)}\n`
      : "";
  return (
    `\nCaveman Stats — Lifetime${window}\n${sep}\n` +
...
function formatStats({
  ...
  const sep = "──────────────────────────────────";
  const shortPath =
    sessionPath && sessionPath.length > 45
...
function main() {
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(
        `caveman-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`,
      );
      process.exit(2);
    }
  ...
  if (parsed.turns > 0) {
    ...
    const suffix =
      agg.estSavedTokens > 0 ? `⛏ ${humanizeTokens(agg.estSavedTokens)}` : "";
  ...
  if (share) {
    ...
    const scanDirs = [claudeDir, process.cwd()].filter(
      (d, i, a) => a.indexOf(d) === i,
    );
    const compressed = summarizeCompressed(findCompressedPairs(scanDirs));
    process.stdout.write(
      formatStats({ ...parsed, mode, sessionPath: sessionFile, compressed }),
    );
  }
}
