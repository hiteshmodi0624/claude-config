...
- Step 0 — constrained query expansion against the graph vocabulary (REQUIRED)
- Step 1 — traversal + save-result feedback loop
- /graphify path — shortest path between two concepts
- /graphify explain — plain-language explanation of one node
- Related: `graphify affected "X"` answers reverse-impact questions ("what depends on X / breaks if X changes"); tune with `--relation` and `--depth`.
