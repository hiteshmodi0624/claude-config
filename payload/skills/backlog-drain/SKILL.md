...
3. **Build + review through the engine's pipeline.** Nothing merges inside the workflow — it returns verified branches. Model routing: builders and reviewers both default to `opus` + effort `high`; drop a stream to `haiku` only when its work is genuinely mechanical.
